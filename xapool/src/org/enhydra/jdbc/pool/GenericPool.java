...
/**
 * GenericPool is the main class of the Pool. It works with any kind of object
...
public class GenericPool {
	...
	/**
	 * checking level object 0 = no special checking 1 = just a check on an
	 * object 2 = test the object 3 = just a check on an object (for all the
	 * objects) 4 = test the object (for all the objects)
	 */
	private int checkLevelObject = 1;
	...
	/**
	 * Generation number. When an error occurs, all objects of the same
	 * generation or earlier are dropped.
	 */
	protected int generation = 1;

    	// Total lifetime the object can exist
    	private long maxLifeTime; 

	// Default Total time the object can be exist.
    	// Default = 0 : can live forever.
    	public static final long DEFAULT_MAXLIFETIME = 0; 
	 ...
	 */
   	public GenericPool(PoolHelper helper) {
        	this(helper, DEFAULT_MINSIZE, DEFAULT_MAXSIZE, DEFAULT_EXPIRATION,
                	DEFAULT_SLEEPTIME, DEFAULT_MAXLIFETIME);
    	}

    	public GenericPool(PoolHelper helper, int initSize) {
        	this(helper, DEFAULT_MINSIZE, initSize, DEFAULT_EXPIRATION,
                	DEFAULT_SLEEPTIME, DEFAULT_MAXLIFETIME);
    	}
	...
	/*
	 * Returns object from the pool or creates a connection outside
	 * synchronization to prevent hanging.
	 */
	...
	public synchronized boolean checkOwner(GenerationObject genObject,
	...
	synchronized public void minimumObject(String user, String password) {
		log
				.debug("GenericPool:minimumObject create object if there are less than minSize objects in the pool count ="
						+ count);
		if ((count < minSize) && (unlocked != null)) { // if pool has less than
			// minSize elements
			long now = System.currentTimeMillis(); // current time

			for (int i = count; i < minSize; i++) { // count have to be equal to
				// minSize
				try {
					...
					log
							.error("GenericPool:minimumObject Error Exception in GenericPool:minimumObject");
				}
			}
			log.debug("GenericPool:minimumObject count=" + count + " Unlocked="
					+ this.getUnlockedObjectCount() + " locked="
	...
	public synchronized void checkIn(Object o) {
		...
		if (count > maxSize) {
			...
			log
					.info("GenericPool:checkIn more than maxSize object in the pool");
			Enumeration enumeration = unlocked.keys(); // get the unlocked pool
			for (int i = maxSize; i < count; i++) { // try to remove object from
				// this pool
				if (getUnlockedObjectCount() > 0) {
					GenerationObject obj = (GenerationObject) enumeration
							.nextElement();
					removeUnlockedObject(obj);
				}
			}
			// now, the size of the pool is changed
			// NOTE : count number ca be greater than maxSize here, in case of
			// there is no
			// available object to delete in the unlocked pool
			count = getUnlockedObjectCount() + getLockedObjectCount();
			if (count > maxSize)
				log
						.warn("GenericPool:checkIn Be careful, the maximum size of the pool does not correspond"
	...
	synchronized public void removeUnlockedObject(GenerationObject obj) {
		--count;
	...
	public synchronized void setMinSize(int min) throws Exception {
		if (min < 0)
			throw new Exception(
					"GenericPool:setMinSize Minimum size of the pool can't be lesser than 0");
		else if (min > maxSize)
			throw new Exception(
			...
			this.minSize = min;
			// minimumObject(); // build object in the pool if it is lesser than
			// minSize
		}
	...
	public synchronized void setMaxSize(int max) throws Exception {
		if (max < 0)
			throw new Exception(
					"GenericPool:setMaxSize Maximum size of the pool can't be lesser than 0");
		else if (max < minSize)
			throw new Exception(
			...
			if (count > max) { // if pool has more than max element
				log
						.info("GenericPool:setMaxSize pool has more than max element");
				...
				for (int i = max; i < count; i++) {
					if (getUnlockedObjectCount() > 0) {
						...
						GenerationObject o = (GenerationObject) enumeration
								.nextElement();
						removeUnlockedObject(o);
					}
				}
				...
				// new size is :
				count = getUnlockedObjectCount() + getLockedObjectCount();
				// if there is still more than max element, the objects will be
				// removed
				// when the check in operation will be performed.
				if (count > max)
					log
							.warn("GenericPool:setMaxSize Be careful, the maximum size of "
	...
	public void setGeneration(int generation) {
		...
		log
				.debug("GenericPool:setGeneration Be careful, it is very dangerous to change "
	...
	/**
	 * Remove unusable objects from the pool, called by PoolKeeper Check the
	...
	protected void cleanUp() {
		...
		if (resize) {
			...
			log
					.info("GenericPool:cleanUp less than minSize objects in the pool "
							+ "min="
							+ minSize
							+ " max="
							+ maxSize
			...
			while (true) {
				try {
					...
					synchronized (this) {
						...
						if (count >= minSize)
							break;
						// we want to make the test inside sync but leave the
						// loop
						// outside sync.
					}

				} catch (Exception e) {
				    log.error("GenericPool:cleanUp   Could not create new connections to fill pool size to minSize.");
				    break; // ADDED THE BREAK SO WE DON't GET AN ENDLESS LOOP WHEN THE DATABASE IS DOWN.
				}
				synchronized (this) {
					notifyAll();
				}
			}
			log.info("GenericPool:cleanUp done " + "min=" + minSize + " max="
					+ maxSize + " count=" + count);
	...
	void expireAll() {
		log
				.debug("GenericPool:expireAll close all object in the unlocked and locked structures");
		for (Enumeration enumeration = unlocked.keys(); enumeration.hasMoreElements();) { // for
			// each
			// object
			// of
	...
	/**
	 * Allows to verify if objects from the pool - for the o generation - are
	...
	synchronized public void removeLockedObject(Object obj) {
		...
		for (Enumeration enumeration = locked.keys(); enumeration.hasMoreElements();) { // for
			...
			GenerationObject o = (GenerationObject) enumeration.nextElement();
			// the locked pool
			System.out.println("GenericPool:removeObject " + o.toString()
					+ " - " + obj.toString());

			if (o.getObj().equals(obj)) {
				System.out.println("GenericPool:removeObject remove found");
	 ...
	 * removes an object for the locked pool, when an error has occurred
	 */
	synchronized public void fullRemoveLockedObject(Object obj) {
		log.debug("GenericPool:removeObject remove an object");
		for (Enumeration enumeration = locked.keys(); enumeration.hasMoreElements();) { // for
			// each
			// object
			...
			if (o.equals(obj)) {
				...
				System.out.println("GenericPool:removeObject remove found");
				poolHelper.expire(o.getObj()); // try to "kill" it
	...
	public Hashtable getUnlockedPool() {
		return unlocked;
	}

	public Hashtable getLockedPool() {
		return locked;
	}
