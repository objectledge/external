...
public class Stemmer {
  ...
  /**
   * Create a Stemmer using stemmer table loaded from resource path pointed to
   * by System property <code>org.getopt.stempel.table</code>. If this
   * property is missing, it is assumed that the included
   * <code>stemmer_2000.out</code> table is to be used.
   */
  public Stemmer() {
    this(System.getProperty("org.getopt.stempel.table",
            "/org/getopt/stempel/tables/stemmer_2000.out"));
  }

  /**
   * Create a Stemmer using selected stemmer table
   * 
   * @param stemmerTable resource path to stemmer table. This resource will be
   *        looked up using this class's ClassLoader.
   */
  ...
  /** Return resource path to the stemmer table, or null if initialized
   * with preloaded table.
   */
   ...
   * @param hideMissing if true, and the stem could not be found, return the
  ...
  /**
   * Testing method. Stemmer table file name is taken from the first argument,
   * and the second argument is the word to stem. If one argument is given,
   * the default table is assumed, and the argument is a word to stem.
   * 
   * @param args
   * @throws Exception
   */
